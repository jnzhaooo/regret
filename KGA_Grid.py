...
class KGA_Grid(DiGraph):
    def __init__(self, PK_WTS, DFA, PS):
        ...
        initial_non_obstacles = set()
        initial_obstacles = set()
        initial_potential_obstacles = PK_WTS.potential_obstacles

        initial_knowledge = (tuple(initial_explored_states), tuple(initial_known_transitions), tuple(initial_non_obstacles), tuple(initial_obstacles), tuple(initial_potential_obstacles))
        ...
        while todo_list:
            ...
            if node in agent_nodes:
                ...
                if x_next in knowledge[0]:
                    ...
                    to_observe_obstacles = set()
                    for neighbor in PK_WTS.successors(x_next):
                        if neighbor in knowledge[4]:
                            to_observe_obstacles.add(neighbor)
                    ...
                    for successor_pattern in PK_WTS.nodes[x_next]['successor_patterns']:
                        ...
                        non_obstacles = set(knowledge[2])
                        obstacles = set(knowledge[3])
                        potential_obstacles = set(knowledge[4])
                        for neighbor in to_observe_obstacles:
                            if neighbor in set(successor_pattern):
                                non_obstacles.add(neighbor)
                                potential_obstacles.remove(neighbor)
                            else:
                                obstacles.add(neighbor)
                                potential_obstacles.remove(neighbor)
                        explored_states.append(x_next)
                        for x_next_next in set(successor_pattern):
                            if x_next_next in to_observe_obstacles:
                                observation_set = set()
                                for neighbor in PK_WTS.successors(x_next_next):
                                    if neighbor not in potential_obstacles:
                                        observation_set.add((x_next_next, neighbor))
                                        observation_set.add((neighbor, x_next_next))
                                known_transitions = known_transitions | observation_set
                        knowledge_next = (tuple(explored_states), tuple(known_transitions), tuple(non_obstacles), tuple(obstacles), tuple(potential_obstacles))
    ...
    def best_response(self, PK_WTS, DFA, knowledge):
        ...
        for edge in PK_WTS.edges:
            if edge[0] in potential_obstacles:
        ...
        if len(node) == 3:
            ...
            for vertex in self.nodes:
                if len(vertex) == 3:
                    ...
                    qq = vertex[1]
                    kknowledge = vertex[2]
                    if x == xx and q == qq and list(knowledge[0]) == list(kknowledge[0]) and set(knowledge[1]) == set(kknowledge[1]) and set(knowledge[2]) == set(kknowledge[2]) and set(knowledge[3]) == set(kknowledge[3]) and set(knowledge[4]) == set(kknowledge[4]):
                        return True, vertex
            return False, None
        else:
            x = node[0]
            ...
            for vertex in self.nodes:
                if len(vertex) == 4:
                    ...
                    kknowledge = vertex[2]
                    xx_next = vertex[3]
                    if x == xx and q == qq and list(knowledge[0]) == list(kknowledge[0]) and set(knowledge[1]) == set(kknowledge[1]) and set(knowledge[2]) == set(kknowledge[2]) and set(knowledge[3]) == set(kknowledge[3]) and set(knowledge[4]) == set(kknowledge[4]) and x_next == xx_next:
                        return True, vertex
            return False, None
